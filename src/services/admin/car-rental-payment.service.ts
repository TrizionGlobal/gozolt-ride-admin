...
export const carRentalPaymentService = {
  async listSettlements(params: { page?: number; limit?: number; status?: string; search?: string }): Promise<SettlementListResponse> {
    try {
      const { data } = await apiClient.get<SettlementListResponse>('/admin/car-rentals/payments/settlements', { params });
      return data;
    } catch {
  ...
  async getKpis(): Promise<PaymentKpis> {
    try {
      const { data } = await apiClient.get<PaymentKpis>('/admin/car-rentals/payments/kpis');
      return data;
    } catch {
      return {
        todayRevenue: 0,
