...
import { FARE_RULE_FIELDS } from '@/services/admin/pricing.types';
...
export function FareRulesCard({ rule, saving, onSave }: FareRulesCardProps) {
  const [isEditing, setIsEditing] = useState(false);
  const [editValues, setEditValues] = useState<Record<string, number>>({});

  const handleEdit = () => {
    const values: Record<string, number> = {};
    FARE_RULE_FIELDS.forEach((f) => {
  ...
  return (
    <div className="rounded-lg border-2 border-dashed border-[#FACC15]/60 bg-[#141414] p-5">
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-base font-semibold text-white">Fare Rules</h3>
        {isEditing ? (
