...
import type { AdminBikeRentalPayout } from '@/hooks/use-admin-bike-rentals';

interface BikeRentalPayoutTableProps {
  data: PaginatedResponse<AdminBikeRentalPayout> | undefined | null;
  loading: boolean;
  page: number;
  limit: number;
...
export function BikeRentalPayoutTable({
  data,
  loading,
  page,
  limit,
  onPageChange,
  onLimitChange,
}: BikeRentalPayoutTableProps) {
  const columns: ColumnDef<AdminBikeRentalPayout>[] = [
    {
      key: 'createdAt',
      title: 'DATE',
      render: (row) => (
