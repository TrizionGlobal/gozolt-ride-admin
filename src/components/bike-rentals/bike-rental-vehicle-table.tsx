...
import type { AdminBikeRentalBike } from '@/hooks/use-admin-bike-rentals';

interface BikeRentalBikeTableProps {
  data: PaginatedResponse<AdminBikeRentalBike> | undefined | null;
  loading: boolean;
  page: number;
  limit: number;
...
const columns: ColumnDef<AdminBikeRentalBike>[] = [
  {
    key: 'bike',
    title: 'VEHICLE',
    render: (row) => (
...
export function BikeRentalVehicleTable({
  data,
  loading,
  page,
  limit,
  onPageChange,
  onLimitChange,
}: BikeRentalBikeTableProps) {
  return (
    <ServerSideTable<AdminBikeRentalBike>
      columns={columns}
      data={data?.data ?? []}
      isLoading={loading}
      page={page}
