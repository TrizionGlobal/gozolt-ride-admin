...
import type { AdminCarRentalBooking } from '@/hooks/use-admin-car-rentals';

interface CarRentalBookingTableProps {
  data: PaginatedResponse<AdminCarRentalBooking> | undefined | null;
  loading: boolean;
  page: number;
  limit: number;
...
export function CarRentalBookingTable({
  data,
  loading,
  page,
  limit,
  onPageChange,
  onLimitChange,
}: CarRentalBookingTableProps) {
  const router = useRouter();

  const columns: ColumnDef<AdminCarRentalBooking>[] = [
    {
      key: 'customer',
      title: 'CUSTOMER',
      render: (row) => (
    ...
    },
    {
      key: 'vehicle',
      title: 'VEHICLE',
      render: (row) => <span className="text-sm text-[#A1A1AA]">{row.vehicle?.name}</span>,
    },
    {
      key: 'dates',
      title: 'DATES',
      ...
      render: (row) => (
        <div className="flex justify-center">
          <button
            onClick={(e) => {
              e.stopPropagation();
              router.push(`/car-rentals/bookings/${row.id}`);
            }}
  ...
  return (
    <ServerSideTable<AdminCarRentalBooking>
      columns={columns}
      data={data?.data ?? []}
      isLoading={loading}
      page={page}
