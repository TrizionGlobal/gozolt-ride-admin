...
import { useCarRentalDashboard } from '@/hooks/use-car-rental-dashboard';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Skeleton } from '@/components/ui/skeleton';

export default function CarRentalDashboardPage() {
  const router = useRouter();
  const { data, loading, refresh } = useCarRentalDashboard();
  ...
  const totalFleet = kpis?.totalVehicles || 1;
      ...
      <div className="rounded-lg border border-[#2A2A2A] bg-[#141414] p-6 space-y-4">
        <div className="flex items-center justify-between border-b border-[#2A2A2A] pb-3">
          <div>
            ...
            <p className="text-xs text-[#6B7280]">Real-time vehicle status across all suppliers</p>
          </div>
          <Link
            href="/car-rentals?tab=vehicles"
            className="text-xs text-[#FFD700] hover:underline flex items-center gap-1 font-semibold"
          >
        ...
        <div className="flex items-center justify-between border-b border-[#2A2A2A] pb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-[#6B7280]">Suppliers independently managing fleets and vehicle handovers</p>
            </div>
          </div>
        </div>
